...
wxBitmap* Imaging::IconToBitmapWithAlpha(const wxIcon& icon) {
  ...
  if (wxBitmap(icon).GetMask()) {
    ...
    // we set the first pixel alpha value to 1 (could be anything
    // as long as it's not 255 or 0). Otherwise we'll get
    // a 1 bit alpha channel which still won't render properly.
    if (image.GetWidth() > 0 && image.GetHeight() > 0) image.SetAlpha(0, 0, 1);
